...
class RelationalDBQueue(IQueue):
    ...
    def get_queue(self):
        users = self.cursor.execute("SELECT users.user_id, preferred_name, ubit, person_num FROM queue INNER JOIN users ON queue.user_id = users.user_id ORDER BY joined")
